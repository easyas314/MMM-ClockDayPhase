...
/* Magic Mirror
 * Module: MMM-ClockDayPhase.js
 *
 * based on clock By Michael Teeuw http://michaelteeuw.nl
 * MIT Licensed.
 */
Module.register("MMM-ClockDayPhase",{
	...
	start: function() {
		...
		// debugging
		console.log(this.config.hoursPhase);

	},

	getPhaseText: function(m) {
		if(!m || !m.isValid()) { return; } //if we can't find a valid or filled moment, we return.
		
		var displayText; // return text
		
		var phaseID;
		phaseID = this.config.hoursPhase[m.hours()];
		if (phaseID == undefined) {
			// NO PHASE FOR THIS HOUR
			return "*"
		}
		displayText = this.config.phaseText[phaseID];
	...
	getDom: function() {
		...
		var phaseWrapper = document.createElement("div");
		...
		phaseWrapper.className = "time bright large light";
		...
		if (this.config.showPhase){
			phaseWrapper.innerHTML = this.getPhaseText(now);
		}
		...
		if (this.config.displayType === "digital") {
			...
			wrapper.appendChild(phaseWrapper);
